...
Do you wish to continue? (y/n)'
    while true; do
        ...
        case $key in
        [Yy])
            sudo rm -rf /var/log/yui
            rm -f /var/tmp/mycroft_web_cache.json
            rm -rf "${TMPDIR:-/tmp}/yui"
            rm -rf "$HOME/.yui"
            sudo rm -rf "/opt/yui"
            exit 0
            ;;
...
if [[ ! -f .dev_opts.json && -z $CI ]] ; then
    echo "
$CYAN                    Welcome to Yui!  $RESET"
    sleep 0.5
    echo '
This script is designed to make working with Yui easy.  During this
...
in yui.conf.
  ...
  N)o, stop the installation."
        if get_YN ; then
            if [[ ! -f /etc/yui/yui.conf ]]; then
                $SUDO mkdir -p /etc/yui
                $SUDO touch /etc/yui/yui.conf
                $SUDO bash -c 'echo "{ \"use_precise\": false }" > /etc/yui/yui.conf'
            else
...
a developer modifying yui-core itself, you should run on the
'master' branch.  It is updated bi-weekly with a stable release.
  ...
  N)o, I want to run unstable branches"
    ...
    sleep 0.5
    echo "
Yui is actively developed and constantly evolving.  It is recommended
that you update regularly.  Would you like to automatically update
whenever launching Yui?  This is highly recommended, especially for
those running against the 'master' branch.
  Y)es, automatically check for updates
  N)o, I will be responsible for keeping Yui updated."
    if get_YN ; then
    ...
    if [[ $opt_forcemimicbuild == false && $opt_skipmimicbuild == false ]] ; then
        sleep 0.5
        echo '
Yui uses its Mimic technology to speak to you.  Mimic can run both
...
on slower machines.  This can be skipped, but Yui will be unable to
...
locally?'
    ...
    echo
    # Add yui-core/bin to the .bashrc PATH?
    sleep 0.5
    echo '
There are several Yui helper commands in the bin folder.  These
can be added to your system PATH, making it simpler to use Yui.
Would you like this to be added to your PATH in the .profile?'
    if get_YN ; then
        echo -e "$HIGHLIGHT Y - Adding Yui commands to your PATH $RESET"
        ...
        if [[ ! -f ~/.profile_mycroft ]] ; then
            ...
            echo '' >> ~/.profile
            echo '# include Yui commands' >> ~/.profile
            echo 'source ~/.profile_mycroft' >> ~/.profile
        fi

        echo "
# WARNING: This file may be replaced in future, do not customize.
# set path so it includes Yui utilities
if [ -d \"${TOP}/bin\" ] ; then
    PATH=\"\$PATH:${TOP}/bin\"
fi" > ~/.profile_mycroft
        echo -e "Type ${CYAN}yui-help$RESET to see available commands."
    else
    ...
    sleep 0.5
    echo
    echo 'The standard location for Yui skills is under /opt/yui/skills.'
    if [[ ! -d /opt/yui/skills ]] ; then
        ...
        $SUDO mkdir -p /opt/yui/skills
        $SUDO chown -R ${setup_user}:${setup_group} /opt/yui
        echo 'Created!'
    fi
    if [[ ! -d skills ]] ; then
        ln -s /opt/yui/skills skills
        echo "For convenience, a soft link has been created called 'skills' which leads"
        echo 'to /opt/yui/skills.'
    fi
...
Yui requires a conflicting package, and will likely uninstall this package.
...
function alpine_install() {
    $SUDO apk add --virtual makedeps-yui-core alpine-sdk git python3 py3-pip py3-setuptools py3-virtualenv mpg123 vorbis-tools pulseaudio-utils fann-dev automake autoconf libtool pcre2-dev pulseaudio-dev alsa-lib-dev swig python3-dev portaudio-dev libjpeg-turbo-dev
}
...
if [[ $disable_precise_later == true ]]; then
    $SUDO bash -c 'jq ". + { \"use_precise\": false }" /etc/yui/yui.conf > tmp.yui.conf' 
                    $SUDO mv -f tmp.yui.conf /etc/yui/yui.conf
fi
...
if [[ ! -x ${VIRTUALENV_ROOT}/bin/activate ]] ; then
    if ! install_venv ; then
        echo 'Failed to set up virtualenv for yui, exiting setup.'
        exit 1
    fi
fi
...
# Add yui-core to the virtualenv path
...
if ! grep -q "$TOP" $VENV_PATH_FILE ; then
    echo 'Adding yui-core to virtualenv path'
    sed -i.tmp '1 a\
'"$TOP"'
' "$VENV_PATH_FILE"
fi
...
chmod +x start-yui.sh
chmod +x stop-yui.sh
chmod +x bin/yui-cli-client
chmod +x bin/yui-help
chmod +x bin/yui-mic-test
chmod +x bin/yui-msk
chmod +x bin/yui-msm
chmod +x bin/yui-pip
chmod +x bin/yui-say-to
chmod +x bin/yui-skill-testrunner
chmod +x bin/yui-speak

# create and set permissions for logging
if [[ ! -w /var/log/yui/ ]] ; then
    # Creating and setting permissions
    echo 'Creating /var/log/yui/ directory'
    if [[ ! -d /var/log/yui/ ]] ; then
        $SUDO mkdir /var/log/yui/
    fi
    $SUDO chmod 777 /var/log/yui/
fi
