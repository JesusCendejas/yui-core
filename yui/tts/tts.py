...
from yui.metrics import report_timing, Stopwatch
...
from yui.util.plugins import load_plugin
...
class TTS(metaclass=ABCMeta):
    ...
    def viseme(self, phonemes):
        ...
        May be implemented to convert TTS phonemes into Yui mouth
        visuals.
    ...
    def clear_cache(self):
        ...
        if not os.path.exists(yui.util.get_cache_directory('tts')):
            return
        for d in os.listdir(yui.util.get_cache_directory("tts")):
            dir_path = os.path.join(yui.util.get_cache_directory("tts"), d)
    ...
    def save_phonemes(self, key, phonemes):
        ...
        cache_dir = yui.util.get_cache_directory("tts/" + self.tts_name)
    ...
    def load_phonemes(self, key):
        ...
        pho_file = os.path.join(
            yui.util.get_cache_directory("tts/" + self.tts_name),
            key + ".pho")
...
def load_tts_plugin(module_name):
    ...
    Args:
        (str) Yui tts module name from config
    Returns:
        class: found tts plugin class
    """
    return load_plugin('yui.plugin.tts', module_name)
...
class TTSFactory:
    ...
    """
    from yui.tts.festival_tts import Festival
    from yui.tts.espeak_tts import ESpeak
    from yui.tts.fa_tts import FATTS
    from yui.tts.google_tts import GoogleTTS
    from yui.tts.mary_tts import MaryTTS
    from yui.tts.mimic_tts import Mimic
    from yui.tts.spdsay_tts import SpdSay
    from yui.tts.bing_tts import BingTTS
    from yui.tts.ibm_tts import WatsonTTS
    from yui.tts.mimic2_tts import Mimic2
    from yui.tts.yandex_tts import YandexTTS
    from yui.tts.dummy_tts import DummyTTS
    from yui.tts.polly_tts import PollyTTS
    from yui.tts.mozilla_tts import MozillaTTS
    ...
    def create():
        ...
        The configuration file ``yui.conf`` contains a ``tts`` section with
