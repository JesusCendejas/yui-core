...
from yui.util.combo_lock import ComboLock
from yui.util.file_utils import get_temp_path
from yui.util.json_helper import load_commented_json, merge_dict
...
def translate_remote(config, setting):
    """Translate config names from server to equivalents for yui-core.
    ...
    for k, v in setting.items():
        if k not in IGNORED_SETTINGS:
            ...
            # Python-style names used within yui-core.
...
def translate_list(config, values):
    """Translate list formated by yui server.

    Args:
        config (dict): target config
        values (list): list from yui server config
    """
    for v in values:
        module = v["@type"]
        if v.get("active"):
...
class RemoteConf(LocalConf):
    ...
    """Config dictionary fetched from yui.ai."""
    def __init__(self, cache=None):
        super(RemoteConf, self).__init__(None)

        cache = cache or join(xdg.BaseDirectory.xdg_cache_home, 'yui',
                              'web_cache.json')
        from yui.api import is_paired
        if not is_paired():
...
def _log_old_location_deprecation():
    LOG.warning("\n ===============================================\n"
                ...
                " Please move it to "
                f"{join(xdg.BaseDirectory.xdg_config_home, 'yui')}")
...
class Configuration:
    ...
    def load_config_stack(configs=None, cache=False, remote=True):
        ...
        Args:
            ...
            remote (boolean): False if the Yui Home settings shouldn't
                              be loaded
        Returns:
        ...
        if not configs:
            ...
            # /etc/xdg/yui/yui.conf
            for conf_dir in xdg.BaseDirectory.load_config_paths('yui'):
                configs.append(LocalConf(join(conf_dir, 'yui.conf')))
            ...
            # Then use the system config (/etc/yui/yui.conf)
