...
from yui.configuration import Configuration, LocalConf
from yui.configuration.locations import OLD_USER_CONFIG
...
class HotWordEngine:
    ...
    """
    def __init__(self, key_phrase="hey yui", config=None, lang="en-us"):
...
class PocketsphinxHotWord(HotWordEngine):
    ...
    def transcribe(self, byte_data, metrics=None):
        ...
        if metrics:
            metrics.timer("yui.stt.local.time_s", time() - start)
...
class PreciseHotword(HotWordEngine):
    """Precise is the default wake word engine for Yui.

    Precise is developed by Yui AI and produces quite good wake word
    ...
    def __init__(self, key_phrase="hey yui", config=None, lang="en-us"):
        ...
        local_conf = LocalConf(
            join(xdg.BaseDirectory.xdg_config_home, 'yui', 'yui.conf')
        )

        for conf_dir in xdg.BaseDirectory.load_config_paths('yui'):
            conf = LocalConf(join(conf_dir, 'yui.conf'))
        ...
        if (local_conf.get('precise', {}).get('dist_url') ==
                'http://bootstrap.yui.ai/artifacts/static/daily/'):
    ...
    @property
    def folder(self):
        old_path = join(expanduser('~'), '.yui', 'precise')
        if os.path.isdir(old_path):
            return old_path
        return xdg.BaseDirectory.save_data_path('yui', 'precise')
    ...
    def __init__(self, key_phrase="hey yui", config=None, lang="en-us"):
        ...
        try:
            ...
            raise Exception(
                ...
                "Please run \"yui-pip install pvporcupine\"") from err
        ...
        LOG.warning('The Porcupine wakeword engine shipped with '
                    'Yui-core is deprecated and will be removed in '
                    'yui-core 21.02. Use the yui-porcupine-plugin '
                    'instead.')
        LOG.info(
...
def load_wake_word_plugin(module_name):
    ...
    Args:
        (str) Yui wake word module name from config
    """
    return load_plugin('yui.plugin.wake_word', module_name)
...
class HotWordFactory:
    ...
    @classmethod
    def create_hotword(cls, hotword="hey yui", config=None,
                       lang="en-us", loop=None):
        if not config:
            config = Configuration.get()['hotwords']
        config = config.get(hotword) or config["hey yui"]
