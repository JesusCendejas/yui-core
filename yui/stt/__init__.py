...
from yui.api import STTApi, HTTPError
...
class STT(metaclass=ABCMeta):
    ...
    def init_language(config_core):
        """Helper method to get language code from Yui config."""
    ...
    def execute(self, audio, language=None):
        ...
        Args:
            audio (AudioData): audio recorded by yui.
...
class YandexSTT(STT):
    """
        ...
        create API key for account and add it to local yui.conf file.
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if e.response.status_code == 401:
                LOG.warning('Access Denied at yui.ai')
...
class GoogleCloudStreamingSTT(StreamingSTT):
    """
        ...
        Google API key to local yui.conf file. The STT config
        will look like this:
...
def load_stt_plugin(module_name):
    ...
    Args:
        module_name (str): Yui stt module name from config
    Returns:
        class: STT plugin class
    """
    return load_plugin('yui.plugin.stt', module_name)
