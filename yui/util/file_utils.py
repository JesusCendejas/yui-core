...
This module contains functions handling yui resource files and things like
accessing and curating yui's cache.
"""
...
def resolve_resource_file(res_name):
    ...
    The system wil look for $XDG_DATA_DIRS/yui/res_name first
    (defaults to ~/.local/share/yui/res_name), and if not found will
    look at /opt/yui/res_name, then finally it will look for res_name
    in the 'yui/res' folder of the source code package.

    Example:
        With yui running as the user 'bob', if you called
        ``resolve_resource_file('snd/beep.wav')``
        it would return either:
        '$XDG_DATA_DIRS/yui/beep.wav',
        '/home/bob/.yui/snd/beep.wav' or
        '/opt/yui/snd/beep.wav' or
        '.../yui/res/snd/beep.wav'
    ...
    """
    config = yui.configuration.Configuration.get()
    ...
    for conf_dir in xdg.BaseDirectory.load_data_paths('yui'):
        filename = os.path.join(conf_dir, res_name)
        if os.path.isfile(filename):
            return filename

    # Now look in the old user location
    filename = os.path.join(os.path.expanduser('~'), '.yui', res_name)
    if os.path.isfile(filename):
        return filename

    # Next look for /opt/yui/res/res_name
...
def get_cache_directory(domain=None):
    ...
    if not directory:
        # If not defined, use /tmp/yui/cache
        directory = get_temp_path('yui', 'cache')
...
def get_temp_path(*args):
    ...
    Example usage: get_temp_path('yui', 'audio', 'example.wav')
    Will return the equivalent of: '/tmp/yui/audio/example.wav'
