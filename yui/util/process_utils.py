...
def create_echo_function(name, whitelist=None):
    """Standard logging mechanism for Yui processes.

    This handles the setup of the basic logging for all Yui
    ...
    if whitelist:
        whitelist.append('yui.debug.log')
    ...
    def echo(message):
        ...
        try:
            ...
            # i.e 'yui.audio.service' will allow the message
            # 'yui.audio.service.play' for example
...
def start_message_bus_client(service, bus=None, whitelist=None):
    ...
    Args:
        ...
        bus (MessageBusClient): an instance of the Yui MessageBusClient
        whitelist (list, optional): List of "type" strings. If defined, only
                                    messages in this list will be logged.
    Returns:
...
class ProcessStatus:
    ...
    Args:
        name (str): process name, will be used to create the messagebus
                    messagetype "yui.{name}...".
        bus (MessageBusClient): Connection to the Yui messagebus.
    ...
    def _register_handlers(self):
        ...
        self.bus.on('yui.{}.is_alive'.format(self.name), self.check_alive)
        self.bus.on('yui.{}.is_ready'.format(self.name),
                    self.check_ready)
        # The next one is for backwards compatibility
        # TODO: remove in 21.02
        self.bus.on(
            'yui.{}.all_loaded'.format(self.name), self.check_ready
        )
