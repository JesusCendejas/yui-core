...
# Verify and present the user with information about their installation of yui.
# Should be run as the yui user.
#
...
function checksysinfo() {
    ...
    mlog " - $( echo "Yui partition disk usage:" && df -h "${MYCROFT_HOME}" )"
...
function checkversion() {
    mlog "Yui version is $( grep -B3 'END_VERSION_BLOCK' ${MYCROFT_HOME}/yui/version/__init__.py | cut -d' ' -f3 | tr -s '\012' '\056' )"
}
...
function checkPIP() {
    ...
    if workon yui ; then
        pip list > /tmp/yui-piplist.$$

        while read reqline ; do
            IFS='==' read  -r -a PIPREQ <<< "$reqline"
            PIPREQVER=$( grep -i ^"${PIPREQ[0]} " /tmp/yui-piplist.$$ | cut -d'(' -f2 | tr -d '\051' )
        ...
        deactivate
        mlog " - PIP list can be found at /tmp/yui-piplist.$$ to verify any issues."
    else
        mlog " = Error: Unable to enter the yui virtualenv, skipping python checks."
    fi
}
...
function checktubes() {
    ...
    case "$( curl -s --max-time 2 -I http://home.yui.ai/ | sed 's/^[^ ]*  *\([0-9]\).*/\1/; 1q' )" in
        [23]) mlog " - HTTP connectivity to https://home.yui.ai worked!";;
        5) mlog " = Error: The web proxy won't let us through to https://home.yui.ai";;
        *) mlog " = Error: The network is down or very slow getting to https://home.yui.ai";;
    esac
}
...
# Where is yui installed?
if [[ -f "${RUNDIR}/yui-service.screen" && -f "${RUNDIR}/../yui/__init__.py" ]] ; then
    MYCROFT_HOME=$(cd "${RUNDIR}" && cd .. && pwd )
else
    if [[ -f "/opt/yui/yui/__init__.py" ]] ; then
        MYCROFT_HOME="/opt/yui/"
    else
        mlog " = Error: Having some difficulty in guessing the home Yui directory?"
        exit 200
    fi
fi

mlog " - Yui appears to be running in ${MYCROFT_HOME}"
