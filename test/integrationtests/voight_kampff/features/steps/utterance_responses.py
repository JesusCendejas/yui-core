...
#
"""
Predefined step definitions for handling dialog interaction with Yui for
use with behave.
"""
...
from yui.dialog import MustacheDialogRenderer
from yui.messagebus import Message
from yui.audio import wait_while_speaking
from yui.util.format import expand_options
...
def then_dialog(context, skill, dialog):
    ...
    assert passed, assert_msg or 'Yui didn\'t respond'
...
@then('yui reply should contain "{text}"')
...
@then('yui should send the message "{message_type}"')
