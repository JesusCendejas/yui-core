...
The module runner can test:
    ...
    That Yui contexts are set or removed
    That the skill speak the intended answer
    The content of any message exchanged between the skill and the yui core
...
from yui.skills.core import YuiSkill, FallbackSkill
from yui.skills.skill_loader import SkillLoader
...
def load_skills(emitter, skills_root):
    ...
    for skill in get_skills(skills_root):
        ...
        from yui.util.log import LOG as skills_log
        buf = StringIO()
...
class MockSkillsLoader(object):
    ...
    def __init__(self, skills_root):
        ...
        from yui.skills.intent_service import IntentService
...
class SkillTest(object):
    ...
    def remove_context(self, s, cxt):
        ...
        if isinstance(cxt, list):
            for x in cxt:
                YuiSkill.remove_context(s, x)
        else:
            YuiSkill.remove_context(s, cxt)

    def set_context(self, s, cxt):
        """Set an adapt context."""
        for key, value in cxt.items():
            YuiSkill.set_context(s, key, value)
    ...
    def execute_test(self, s):
        ...
        Args:
            s (MycroftSkill): yui skill to test
    ...
    def check_queue(self, q, evaluation_rule):
        ...
        try:
            ...
            if event.msg_type == 'yui.skill.handler.complete':
    ...
    def shutdown_emitter(self, s):
        ...
        self.emitter.remove_all_listeners('yui.skill.handler.complete')
