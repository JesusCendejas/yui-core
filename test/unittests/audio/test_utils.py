...
import yui.audio
from yui.util import create_signal, check_for_signal
...
def wait_while_speaking_thread():
    global done_waiting
    yui.audio.wait_while_speaking()
    done_waiting = True


class TestInterface(unittest.TestCase):
    def setUp(self):
        if exists(get_temp_path('yui')):
            rmtree(get_temp_path('yui'))

    def test_is_speaking(self):
        create_signal('isSpeaking')
        self.assertTrue(yui.audio.is_speaking())
        # Check that the signal hasn't been removed
        self.assertTrue(check_for_signal('isSpeaking'))
        self.assertFalse(yui.audio.is_speaking())
    ...
    @mock.patch('yui.audio.utils.is_speaking')
    @mock.patch('yui.messagebus.send_func.send')
    def test_stop_speaking(self, mock_send, mock_is_speaking):
        ...
        yui.audio.stop_speaking()
        mock_send.assert_called_with('yui.audio.speech.stop')
