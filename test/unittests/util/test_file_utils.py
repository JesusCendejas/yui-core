...
from yui import MYCROFT_ROOT_PATH
...
@mock.patch('yui.configuration.Configuration')
class TestResolveResource(TestCase):
    ...
    def test_source_package(self, mock_conf):
        """Check file shipped in the yui res folder."""
        mock_conf.get.return_value = test_config
        expected_path = join(MYCROFT_ROOT_PATH, 'yui', 'res',
                             'text', 'en-us', 'and.word')
...
class TestCache(TestCase):
    ...
    def test_get_cache_directory(self, mock_conf):
        mock_conf.get.return_value = test_config
        expected_path = join(test_config['cache_dir'], 'yui', 'cache')
    ...
    def test_get_cache_directory_with_domain(self, mock_conf):
        mock_conf.get.return_value = test_config
        expected_path = join(test_config['cache_dir'], 'yui',
                             'cache', 'whales')
        self.assertEqual(get_cache_directory('whales'), expected_path)
        self.assertTrue(isdir(expected_path))

    @mock.patch('yui.util.file_utils.psutil')
...
class TestCreateFile(TestCase):
    ...
    def test_get_temp_path(self):
        ...
        temp_path = get_temp_path('yui', 'audio', 'example.wav')
        self.assertEqual(temp_path, temp_dir + '/yui/audio/example.wav')
