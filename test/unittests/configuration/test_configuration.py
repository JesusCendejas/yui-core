...
class TestConfiguration(TestCase):
    def setUp(self):
        ...
        yui.configuration.Configuration.load_config_stack([{}], True)

    def test_get(self):
        d1 = {'a': 1, 'b': {'c': 1, 'd': 2}}
        d2 = {'b': {'d': 'changed'}}
        d = yui.configuration.Configuration.get([d1, d2])
        self.assertEqual(d['a'], d1['a'])
        self.assertEqual(d['b']['d'], d2['b']['d'])
        self.assertEqual(d['b']['c'], d1['b']['c'])

    @patch('yui.api.DeviceApi')
    def test_remote(self, mock_api):
        ...
        rc = yui.configuration.RemoteConf()
        self.assertTrue(rc['test_config'])
        self.assertEqual(rc['location']['city']['name'], 'Stockholm')

    @patch('json.dump')
    @patch('yui.configuration.config.exists')
    @patch('yui.configuration.config.isfile')
    @patch('yui.configuration.config.load_commented_json')
    def test_local(self, mock_json_loader, mock_isfile, mock_exists,
        ...
        lc = yui.configuration.LocalConf('test')
    ...
    @patch('yui.configuration.config.RemoteConf')
    @patch('yui.configuration.config.LocalConf')
    def test_update(self, mock_remote, mock_local):
        mock_remote.return_value = {}
        mock_local.return_value = {'a': 1}
        c = yui.configuration.Configuration.get()
        self.assertEqual(c, {'a': 1})

        mock_local.return_value = {'a': 2}
        yui.configuration.Configuration.updated('message')
