...
class PocketSphinxRecognizerTest(unittest.TestCase):
    def setUp(self):
        with patch('yui.configuration.Configuration.get') as \
                mock_config_get:
            conf = base_config()
            conf['hotwords']['hey yui']['module'] = 'pocketsphinx'
    ...
    def testRecognizerWrapper(self):
        ...
        source = WavFile(os.path.join(DATA_DIR, "yui.wav"))
        with source as audio:
            assert self.recognizer.found_wake_word(audio.stream.read())
...
class LocalRecognizerInitTest(unittest.TestCase):
    ...
    def testListenerConfig(self, mock_config_get):
        ...
        # Test "Hey Yui"
        rl = RecognizerLoop()
        self.assertEqual(rl.wakeword_recognizer.key_phrase, "hey yui")
