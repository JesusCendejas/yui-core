...
from yui.skills.skill_data import (load_regex_from_file, load_regex,
                                       load_vocabulary, read_vocab_file)
from yui.skills.core import MycroftSkill, resting_screen_handler
from yui.skills.intent_service import open_intent_envelope
...
class TestMycroftSkill(unittest.TestCase):
    ...
    def _mock_local_settings(self):
        local_settings_patch = patch(
            'yui.skills.mycroft_skill.mycroft_skill.get_local_settings'
        )
    ...
    def test_translate_locations(self):
        ...
        s.root_dir = abspath(join(dirname(__file__),
                                  'translate', 'in-locale'))
        lst = s.translate_list('good_things')
        self.assertEqual(lst, ['sonne', 'yui', 'zahne'])
        vals = s.translate_namedvalues('named_things')
