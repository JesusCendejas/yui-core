...
from yui.messagebus import Message
from yui.skills.audioservice import AudioService
...
class TestAudioServiceControls(TestCase):
    ...
    def test_pause(self):
        ...
        self.assertLastMessageTypeEqual(self.bus,
                                        'yui.audio.service.pause')

    def test_resume(self):
        self.audioservice.resume()
        self.assertLastMessageTypeEqual(self.bus,
                                        'yui.audio.service.resume')

    def test_next(self):
        self.audioservice.next()
        self.assertLastMessageTypeEqual(self.bus, 'yui.audio.service.next')

    def test_prev(self):
        self.audioservice.prev()
        self.assertLastMessageTypeEqual(self.bus, 'yui.audio.service.prev')

    def test_stop(self):
        self.audioservice.stop()
        self.assertLastMessageTypeEqual(self.bus, 'yui.audio.service.stop')

    def test_seek(self):
        self.audioservice.seek()
        message = self.bus.emit.call_args_list[-1][0][0]
        self.assertEqual(message.msg_type,
                         'yui.audio.service.seek_forward')
        ...
        message = self.bus.emit.call_args_list[-1][0][0]
        self.assertEqual(message.msg_type,
                         'yui.audio.service.seek_backward')
        self.assertEqual(message.data['seconds'], 5)
...
class TestAudioServicePlay(TestCase):
    ...
    def test_proper_uri(self):
        ...
        message = self.bus.emit.call_args_list[-1][0][0]
        self.assertEqual(message.msg_type, 'yui.audio.service.play')
...
class TestAudioServiceQueue(TestCase):
    ...
    def test_uri(self):
        ...
        message = self.bus.emit.call_args_list[-1][0][0]
        self.assertEqual(message.msg_type, 'yui.audio.service.queue')
        self.assertEqual(message.data['tracks'], ['file:///hello_nasty.mp3'])
