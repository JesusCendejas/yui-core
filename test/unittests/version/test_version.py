...
from yui.version import check_version, CORE_VERSION_STR, VersionManager
...
class TestVersion(unittest.TestCase):
    @patch('yui.version.CORE_VERSION_TUPLE', (0, 8, 20))
    def test_get_version(self):
        """
            Tests for yui.version.get_version()
    ...
    @patch('yui.version.isfile')
    @patch('yui.version.exists')
    @patch('yui.version.open',
           mock_open(read_data=VERSION_INFO), create=True)
    def test_version_manager_get(self, mock_exists, mock_isfile):
        """Test yui.version.VersionManager.get()
