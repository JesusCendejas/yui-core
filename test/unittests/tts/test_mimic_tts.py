...
from yui.tts.mimic_tts import (Mimic, download_subscriber_voices,
                                   get_mimic_binary,
...
@mock.patch('yui.tts.mimic_tts.DeviceApi')
@mock.patch('yui.tts.tts.PlaybackThread')
class TestMimic(unittest.TestCase):
    @mock.patch('yui.tts.mimic_tts.subprocess')
    ...
    @mock.patch('yui.tts.mimic_tts.Thread')
    def test_subscriber(self, mock_thread, _, mock_device_api):
        ...
        with mock.patch('yui.tts.mimic_tts.exists') as mock_exists:
            mock_exists.return_value = True
            self.assertEqual(m.args, [trinity_mimic, '-voice',
                                      'trinity', '-psdur', '-ssml'])

    @mock.patch('yui.tts.mimic_tts.sleep')
    @mock.patch('yui.tts.mimic_tts.download')
    def test_download(self, mock_download, mock_sleep, _, mock_device_api):
        ...
        self.assertEqual(mock_download.call_args[0][:2],
                         ('https://trinity', '/opt/yui/voices/mimic_tn'))
        make_executable = mock_download.call_args[0][2]

        # Check that the excutable flag is set to the downloaded file
        with mock.patch('yui.tts.mimic_tts.os.chmod') as mock_chmod:
            with mock.patch('yui.tts.mimic_tts.os.stat') as mock_stat:
                st_mock = mock.Mock()
