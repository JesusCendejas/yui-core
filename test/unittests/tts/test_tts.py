...
class MockTTSValidator(yui.tts.TTSValidator):
    def validate(self):
        pass
...
class TestPlaybackThread(unittest.TestCase):
    def test_lifecycle(self):
        playback = yui.tts.PlaybackThread(Queue())
        playback.init(mock.Mock())
        playback.start()
        playback.stop()
        playback.join()

    @mock.patch('yui.tts.tts.time')
    @mock.patch('yui.tts.tts.play_wav')
    @mock.patch('yui.tts.tts.play_mp3')
...
@mock.patch('yui.tts.tts.PlaybackThread')
class TestTTS(unittest.TestCase):
    def test_execute(self, mock_playback_thread):
        ...
        tts.queue = mock.Mock()
        with mock.patch('yui.tts.tts.open') as mock_open:
    ...
    def test_phoneme_cache(self, mock_open, _):
        ...
        with mock.patch('yui.tts.tts.os.path.exists') as _:
            read_phonemes = tts.load_phonemes('abc')
    ...
    def test_ssml_support(self, _):
        ...
        self.assertEqual(yui.tts.TTS.remove_ssml(sentence),
                         sentence_no_ssml)
...
class TestTTSFactory(unittest.TestCase):
    @mock.patch('yui.tts.tts.Configuration')
    def test_create(self, mock_config):
        ...
        yui.tts.TTSFactory.CLASSES['mimic'] = mock_mimic
        yui.tts.TTSFactory.CLASSES['mock'] = mock_tts_class

        # Check that correct module is selected
        tts_instance = yui.tts.TTSFactory.create()
