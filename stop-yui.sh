...
function process-running() {
    if [[ $( pgrep -f "python3 (.*)-m yui.*${1}" ) ]] ; then
        return 0
    else
        return 1
    fi
...
case ${OPT} in
    "all")
        ;&
    "")
        echo "Stopping all yui-core services"
        end-process skills
        end-process audio
        end-process speech
