...
from .yui_skill.event_container import EventContainer, create_basic_wrapper
...
class EventScheduler(Thread):
    ...
    Args:
        bus:            Yui messagebus (yui.messagebus)
    ...
    def __init__(self, bus, schedule_file='schedule.json'):
        ...
        new_schedule_path = join(
            xdg.BaseDirectory.load_first_config('yui'), schedule_file
        )
        ...
        self.bus.on('yui.scheduler.schedule_event',
                    self.schedule_event_handler)
        self.bus.on('yui.scheduler.remove_event',
                    self.remove_event_handler)
        self.bus.on('yui.scheduler.update_event',
                    self.update_event_handler)
        self.bus.on('yui.scheduler.get_event',
    ...
    def get_event_handler(self, message):
        ...
        emitter_name = 'yui.event_status.callback.{}'.format(event_name)
    ...
    def shutdown(self):
        ...
        # Remove listeners
        self.bus.remove_all_listeners('yui.scheduler.schedule_event')
        self.bus.remove_all_listeners('yui.scheduler.remove_event')
        self.bus.remove_all_listeners('yui.scheduler.update_event')
...
class EventSchedulerInterface:
    ...
    def _schedule_event(self, handler, when, data, name,
        ...
        event_data = {'time': time.mktime(when.timetuple()),
                      ...
                      'data': data}
        self.bus.emit(Message('yui.scheduler.schedule_event',
    ...
    def update_scheduled_event(self, name, data=None):
        ...
        self.bus.emit(Message('yui.schedule.update_event',
                              data=data))
    ...
    def cancel_scheduled_event(self, name):
        ...
        if self.events.remove(unique_name):
            self.bus.emit(Message('yui.scheduler.remove_event',
                                  data=data))
    ...
    def get_scheduled_event_status(self, name):
        ...
        reply_name = 'yui.event_status.callback.{}'.format(event_name)
        msg = Message('yui.scheduler.get_event', data=data)
