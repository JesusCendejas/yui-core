...
#
"""Common logic for instantiating the Yui Skills Manager.

The Yui Skills Manager (MSM) does a lot of interactions with git.  The
...
def _init_msm_lock():
    msm_lock = None
    try:
        msm_lock = ComboLock(get_temp_path('yui-msm.lck'))
        LOG.debug('yui-msm combo lock instantiated')
    except Exception:
