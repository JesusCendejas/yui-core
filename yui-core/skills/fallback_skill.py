...
from .yui_skill import YuiSkill, get_handler_name


class FallbackSkill(YuiSkill):
    ...
    ========   ========   ================================================
       1-4     RESERVED   Unused for now, slot for pre-Padatious if needed
         5     YUI    Padatious near match (conf > 0.8)
      6-88     USER       General
        89     YUI    Padatious loose match (conf > 0.5)
     90-99     USER       Uncaught intents
       100+    YUI    Fallback Unknown or other future use
    ========   ========   ================================================
    ...
    def make_intent_failure_handler(cls, bus):
        ...
        def handler(message):
            ...
            LOG.debug('Checking fallbacks in range '
                      '{} - {}'.format(start, stop))
            bus.emit(message.forward("yui.skill.handler.start",
            ...
            with stopwatch:
                ...
                for handler in handlers:
                    ...
                    bus.emit(message.forward('yui.skill.handler.complete',
                                             data={'handler': "fallback",
                                                   'exception': warning}))
