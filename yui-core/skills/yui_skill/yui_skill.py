...
#
"""Common functionality relating to the implementation of yui skills."""
...
from yui import dialog
from yui.api import DeviceApi
from yui.audio import wait_while_speaking
from yui.enclosure.api import EnclosureAPI
from yui.enclosure.gui import SkillGUI
from yui.configuration import Configuration
from yui.dialog import load_dialogs
from yui.filesystem import FileSystemAccess
from yui.messagebus.message import Message, dig_for_message
from yui.metrics import report_metric
...
from yui.util.format import pronounce_number, join_list
from yui.util.parse import match_one, extract_number
...
def get_non_properties(obj):
    ...
    def check_class(cls):
        ...
        for b in [b for b in cls.__bases__ if b not in (object, YuiSkill)]:
            np += check_class(b)
        return np
...
class YuiSkill:
    """Base class for yui skills providing common behaviour and parameters
    to all Skill implementations.

    For information on how to get started with creating yui skills see
    https://yui.ai/documentation/skills/introduction-developing-skills/
    ...
    def __init__(self, name=None, bus=None, use_settings=True):
        ...
        #: directory. E.g. /opt/yui/skills/my-skill.me/
        ...
        #: See yui.filesystem for details.
    ...
    def _init_settings(self):
        ...
        if not self.settings_write_path.joinpath('settings.json').exists():
            self.settings_write_path = Path(BaseDirectory.save_config_path(
                'yui', 'skills', basename(self.root_dir)))
        ...
        if not settings_read_path.joinpath('settings.json').exists():
            for dir in BaseDirectory.load_config_paths('yui',
                                                       'skills',
                                                       basename(
                                                           self.root_dir)):
                path = Path(dir)
    ...
    def _register_system_event_handlers(self):
        """Add all events allowing the standard interaction with the Yui
        system.
        """
        ...
        if stop_is_implemented():
            self.add_event('yui.stop', self.__handle_stop)

        self.add_event(
            'yui.skill.enable_intent',
            self.handle_enable_intent
        )
        self.add_event(
            'yui.skill.disable_intent',
            self.handle_disable_intent
        )
        self.add_event(
            'yui.skill.set_cross_context',
            self.handle_set_cross_context
        )
        self.add_event(
            'yui.skill.remove_cross_context',
            self.handle_remove_cross_context
        )
        self.events.add(
            'yui.skills.settings.changed',
    ...
    def handle_settings_change(self, message):
        ...
        A "yui.skills.settings.changed" event is emitted for each skill
    ...
    def get_response(self, dialog='', data=None, validator=None,
        ...
        if dialog_exists:
            ...
            self.bus.emit(Message('yui.mic.listen'))
    ...
    def voc_match(self, utt, voc_filename, lang=None, exact=False):
        ...
        in the "res/text" folder of yui-core. The result is cached to
        ...
        if cache_key not in self.voc_match_cache:
            # Check for both skill resources and yui-core resources
            voc = self.find_resource(voc_filename + '.voc', 'vocab')
            if not voc:  # Check for vocab in yui core resources
    ...
    def report_metric(self, name, data):
        """Report a skill metric to the Yui servers.
    ...
    def register_resting_screen(self):
        ...
        for attr_name in get_non_properties(self):
            ...
            if hasattr(method, 'resting_handler'):
                ...
                self.add_event('yui.mark2.collect_idle',
    ...
    def register_intent_file(self, intent_file, handler):
        ...
        if handler:
            self.add_event(name, handler, 'yui.skill.handler')
    ...
    def set_cross_skill_context(self, context, word=''):
        ...
        Args:
            ...
            word:       word connected to keyword
        """
        self.bus.emit(Message('yui.skill.set_cross_context',
    ...
    def remove_cross_skill_context(self, context):
        ...
        if not isinstance(context, str):
            raise ValueError('context should be a string')
        self.bus.emit(Message('yui.skill.remove_cross_context',
                              {'context': context}))
    ...
    def speak(self, utterance, expect_response=False, wait=False, meta=None):
        ...
        Args:
            utterance (str):        sentence yui should speak
            expect_response (bool): set to True if Yui should listen
    ...
    def __handle_stop(self, _):
        """Handler for the "yui.stop" signal. Runs the user defined
        `stop()` method.
        """
        def __stop_timeout():
