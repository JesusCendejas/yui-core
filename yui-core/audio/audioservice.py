...
from yui.messagebus.message import Message
from yui.util.log import LOG
from yui.util.monotonic_event import MonotonicEvent
from yui.util.plugins import find_plugins
...
def setup_service(service_module, config, bus):
    ...
    Args:
        ...
        config (dict): Yui configuration dict
...
def load_internal_services(config, bus, path=None):
    """Load audio services included in Yui-core.
...
def load_plugins(config, bus):
    ...
    """
    plugin_services = []
    found_plugins = find_plugins('yui.plugin.audioservice')
...
def load_services(config, bus, path=None):
    ...
    "yui.plugin.audioservice" entrypoint group.
...
class AudioService:
    ...
    def load_services(self):
        ...
        self.bus.on('yui.audio.service.play', self._play)
        self.bus.on('yui.audio.service.queue', self._queue)
        self.bus.on('yui.audio.service.pause', self._pause)
        self.bus.on('yui.audio.service.resume', self._resume)
        self.bus.on('yui.audio.service.stop', self._stop)
        self.bus.on('yui.audio.service.next', self._next)
        self.bus.on('yui.audio.service.prev', self._prev)
        self.bus.on('yui.audio.service.track_info', self._track_info)
        self.bus.on('yui.audio.service.list_backends', self._list_backends)
        self.bus.on('yui.audio.service.seek_forward', self._seek_forward)
        self.bus.on('yui.audio.service.seek_backward', self._seek_backward)
    ...
    def track_start(self, track):
        ...
        if track:
            ...
            self.bus.emit(Message('yui.audio.playing_track',
                                  data={'track': track}))
        else:
            # If no track is about to start last track of the queue has been
            # played.
            LOG.debug('End of playlist!')
            self.bus.emit(Message('yui.audio.queue_end'))

    def _pause(self, message=None):
        """
            Handler for yui.audio.service.pause. Pauses the current audio
            service.
    ...
    def _resume(self, message=None):
        """
            Handler for yui.audio.service.resume.
    ...
    def _next(self, message=None):
        """
            Handler for yui.audio.service.next. Skips current track and
    ...
    def _prev(self, message=None):
        """
            Handler for yui.audio.service.prev. Starts playing the previous
            track.
    ...
    def _perform_stop(self):
        ...
        if self.current:
            ...
            if self.current.stop():
                self.bus.emit(Message("yui.stop.handled",
                                      {"by": "audio:" + name}))
    ...
    def _stop(self, message=None):
        """
            Handler for yui.stop. Stops any playing service.
    ...
    def _lower_volume(self, message=None):
        """
            Is triggered when yui starts to speak and reduces the volume.
    ...
    def _restore_volume(self, _=None):
        """Triggered when yui is done speaking and restores the volume."""
    ...
    def _restore_volume_after_record(self, message=None):
        """
            Restores the volume when Yui is done recording.
    ...
    def _play(self, message):
        """
            Handler for yui.audio.service.play. Starts playback of a
    ...
    def _track_info(self, message):
        ...
        if self.current:
            ...
            track_info = {}
        self.bus.emit(Message('yui.audio.service.track_info_reply',
                              data=track_info))
    ...
    def shutdown(self):
        ...
        # remove listeners
        self.bus.remove('yui.audio.service.play', self._play)
        self.bus.remove('yui.audio.service.queue', self._queue)
        self.bus.remove('yui.audio.service.pause', self._pause)
        self.bus.remove('yui.audio.service.resume', self._resume)
        self.bus.remove('yui.audio.service.stop', self._stop)
        self.bus.remove('yui.audio.service.next', self._next)
        self.bus.remove('yui.audio.service.prev', self._prev)
        self.bus.remove('yui.audio.service.track_info', self._track_info)
        self.bus.remove('yui.audio.service.seek_forward',
                        self._seek_forward)
        self.bus.remove('yui.audio.service.seek_backward',
                        self._seek_backward)
