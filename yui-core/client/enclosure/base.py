...
from yui.util import create_daemon, start_message_bus_client
...
class Enclosure:
    def __init__(self):
        ...
        self.bus.on("yui.gui.connected", self.on_gui_client_connected)
    ...
    def on_gui_send_event(self, message):
        ...
        try:
            data = {'type': 'yui.events.triggered',
    ...
    def __insert_pages(self, namespace, pages):
        ...
        self.send({"type": "yui.gui.list.insert",
                   "namespace": namespace,
    ...
    def __remove_page(self, namespace, pos):
        ...
        self.send({"type": "yui.gui.list.remove",
                   "namespace": namespace,
                   "position": pos,
                   "items_number": 1
                   })
        ...
        if (pos == 0 and len(self.loaded[0].pages) == 0):
            self.bus.emit(Message("yui.device.show.idle"))
    ...
    def __insert_new_namespace(self, namespace, pages):
        ...
        self.send({"type": "yui.session.list.insert",
                   "namespace": "yui.system.active_skills",
                   "position": 0,
    ...
    def __move_namespace(self, from_pos, to_pos):
        ...
        if self.explicit_move:
            ...
            self.send({"type": "yui.session.list.move",
                       "namespace": "yui.system.active_skills",
    ...
    def __switch_page(self, namespace, pages):
        ...
        self.send({"type": "yui.events.triggered",
                   "namespace": namespace,
    ...
    def remove_namespace(self, namespace):
        ...
        if index is None:
            ...
            self.send({"type": "yui.session.list.remove",
                       "namespace": "yui.system.active_skills",
                       "position": index,
                       "items_number": 1
                       })
    ...
    # 2) Yui prepares a port for a socket connection to this GUI
    ...
    def on_gui_client_connected(self, message):
        ...
        self.bus.emit(Message("yui.gui.port",
                              {"port": port,
                               "gui_id": gui_id}))
...
def create_gui_service(enclosure, config):
    ...
    # Disable all tornado logging so yui loglevel isn't overridden
...
class GUIWebsocketHandler(WebSocketHandler):
    """The socket pipeline between the GUI and Yui."""
    clients = []
    ...
    def synchronize(self):
        ...
        for namespace, pages in enclosure.loaded:
            ...
            for key in data:
                self.send({"type": "yui.session.set",
                           "namespace": namespace,
                           "data": {key: data[key]}
                           })
            namespace_pos += 1
    ...
    def on_message(self, message):
        ...
        if (msg.get('type') == "yui.events.triggered" and
