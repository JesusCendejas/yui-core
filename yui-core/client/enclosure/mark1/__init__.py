...
import yui.dialog
from yui.client.enclosure.base import Enclosure
from yui.api import has_been_paired
from yui.audio import wait_while_speaking
from yui.client.enclosure.mark1.arduino import EnclosureArduino
from yui.client.enclosure.mark1.eyes import EnclosureEyes
from yui.client.enclosure.mark1.mouth import EnclosureMouth
from yui.enclosure.display_manager import \
    init_display_manager_bus_connection
from yui.configuration import LocalConf, USER_CONFIG
from yui.messagebus.message import Message
from yui.util import play_wav, create_signal, connected, check_for_signal
from yui.util.audio_test import record
from yui.util.log import LOG
from queue import Queue
from yui.util.file_utils import get_temp_path
...
class EnclosureReader(Thread):
    ...
    E.g. Yui Stop Feature
        # . Arduino sends a Stop command after a button press on a Yui unit
        # . ``EnclosureReader`` captures the Stop command
        # . Notify all Yui Core processes (e.g. skills) to be stopped
    ...
    def __init__(self, serial, bus, lang=None):
        ...
        # Notifications from yui-core
        self.bus.on("yui.stop.handled", self.on_stop_handled)
    ...
    def process(self, data):
        ...
        if "yui.stop" in data:
            if has_been_paired():
                ...
                self.bus.emit(Message("yui.stop"))

        if "volume.up" in data:
            self.bus.emit(Message("yui.volume.increase",
                                  {'play_sound': True}))

        if "volume.down" in data:
            self.bus.emit(Message("yui.volume.decrease",
                                  {'play_sound': True}))
        ...
        if "unit.factory-reset" in data:
            self.bus.emit(Message("speak", {
                'utterance': yui.dialog.get("reset to factory defaults")}))
            ...
            subprocess.call(
                'rm ~/.yui/identity/identity2.json',
                shell=True)
        ...
        if "unit.enable-ssh" in data:
            ...
            self.bus.emit(Message("speak", {
                'utterance': yui.dialog.get("ssh enabled")}))

        if "unit.disable-ssh" in data:
            # This is handled by the wifi client
            self.bus.emit(Message("system.ssh.disable"))
            self.bus.emit(Message("speak", {
                'utterance': yui.dialog.get("ssh disabled")}))
        ...
        if "unit.enable-learning" in data or "unit.disable-learning" in data:
            ...
            self.bus.emit(Message("speak", {
                'utterance': yui.dialog.get("learning " + word)}))
...
class EnclosureWriter(Thread):
    ...
    Writes data to Serial port.
        # . Enqueues all commands received from Yui enclosures
           implementation
        # . Process them on the received order by writing on the Serial port

    E.g. Displaying a text on Yui's Mouth
...
class EnclosureMark1(Enclosure):
    """
    Serves as a communication interface between Arduino and Yui Core.
    ...
    def _handle_pairing_complete(self, Message):
        """
            Handler for 'yui.paired', unmutes the mic after the pairing is
            complete.
        """
        self.bus.emit(Message("yui.mic.unmute"))
    ...
    def _do_net_check(self):
        ...
        if not connected():  # and self.conn_monitor is None:
            if has_been_paired():
                ...
                self.bus.emit(Message("yui.mic.mute"))
                # Setup handler to unmute mic at the end of on boarding
                # i.e. after pairing is complete
                self.bus.once('yui.paired', self._handle_pairing_complete)

                self.speak(yui.dialog.get('yui.intro'))
                wait_while_speaking()
