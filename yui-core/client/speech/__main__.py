...
from yui.client.speech.listener import RecognizerLoop
from yui.configuration import Configuration
from yui.identity import IdentityManager
from yui.lock import Lock as PIDLock  # Create/Support PID locking file
...
from yui.util.process_utils import ProcessStatus, StatusCallbackMap
...
def handle_awoken():
    """Forward yui.awoken to the messagebus."""
    LOG.info("Listener is now Awake: ")
    context = {'client_name': 'mycroft_listener',
               'source': 'audio'}
    bus.emit(Message('yui.awoken', context=context))
...
def handle_unknown():
    context = {'client_name': 'mycroft_listener',
               'source': 'audio'}
    bus.emit(Message('yui.speech.recognition.unknown', context=context))
...
def handle_stop(event):
    """Handler for yui.stop, i.e. button press."""
    loop.force_unmute()
...
def connect_bus_events(bus):
    # Register handlers for events on main Yui messagebus
    ...
    bus.on('yui.mic.mute', handle_mic_mute)
    bus.on('yui.mic.unmute', handle_mic_unmute)
    bus.on('yui.mic.get_status', handle_mic_get_status)
    bus.on('yui.mic.listen', handle_mic_listen)
    bus.on("yui.paired", handle_paired)
    bus.on('recognizer_loop:audio_output_start', handle_audio_start)
    bus.on('recognizer_loop:audio_output_end', handle_audio_end)
    bus.on('yui.stop', handle_stop)
