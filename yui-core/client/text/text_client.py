...
from yui.messagebus.client import MessageBusClient
...
bSimple = False
bus = None  # Yui messagebus connection
config = {}  # Will be populated by the Yui configuration
event_thread = None
history = []
...
def load_mycroft_config(bus):
    """ Load the yui config and connect it to updates over the messagebus.
    """
...
def connect_to_mycroft():
    """ Connect to the yui messagebus and load and register config
        on the bus.
...
def load_settings():
    ...
    if os.path.isfile(path):
        ...
        LOG.warning(" Please move it to " +
                    os.path.join(xdg.BaseDirectory.xdg_config_home, 'yui',
                                 filename))
        config_file = path

    # Check XDG_CONFIG_DIR
    if config_file is None:
        for conf_dir in xdg.BaseDirectory.load_config_paths('yui'):
    ...
    # Check /etc/yui
    if config_file is None:
        config_file = os.path.join("/etc/yui", filename)
...
def save_settings():
    ...
    config_file = os.path.join(
        xdg.BaseDirectory.save_config_path("yui"), filename)
...
def connect(bus):
    """ Run the yui messagebus referenced by bus.

        Args:
            bus:    Yui messagebus instance
    """
    bus.run_forever()
...
def do_draw_main(scr):
    ...
    if find_str:
        ...
        scr.addstr(0, 0, "Log Output:" + " " * (curses.COLS - 31) +
                   str(start) + "-" + str(end) + " of " + str(cLogs),
                   CLR_HEADING)
    ver = " yui-core " + yui.version.CORE_VERSION_STR + " ==="
...
def do_draw_help(scr):
    ...
    def render_header():
        scr.addstr(0, 0, center(25) + "Yui Command Line Help", CLR_HEADING)
...
def handle_cmd(cmd):
    ...
    if "show" in cmd and "log" in cmd:
        ...
        # Control logging behavior in all Yui processes
        if "level" in cmd:
            level = _get_cmd_param(cmd, ["log", "level"])
            bus.emit(Message("yui.debug.log", data={'level': level}))
        elif "bus" in cmd:
            state = _get_cmd_param(cmd, ["log", "bus"]).lower()
            if state in ["on", "true", "yes"]:
                bus.emit(Message("yui.debug.log", data={'bus': True}))
            elif state in ["off", "false", "no"]:
                bus.emit(Message("yui.debug.log", data={'bus': False}))
    elif "history" in cmd:
        # extract last word(s)
        ...
        message = bus.wait_for_response(
            Message('skillmanager.list'), reply_type='yui.skills.list')
...
def gui_main(stdscr):
    ...
    add_log_message("Establishing Yui Messagebus connection...")
    ...
    try:
        while True:
            ...
            if code == 27:    # Hitting ESC twice clears the entry line
                ...
                # ENTER sends the typed line to be processed by Yui
                if line == "":
                    continue
...
def connect_to_messagebus():
    """ Connect to the yui messagebus and launch a thread handling the
        connection.

        Returns: WebsocketClient
    """
    bus = MessageBusClient()  # Yui messagebus connection
