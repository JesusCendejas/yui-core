...
function help() {
    echo "${script}:  Yui command/service launcher"
    ...
    echo "  unittest                 run yui-core unit tests (requires pytest)"
...
function name-to-script-path() {
    case ${1} in
        "bus")               _module="yui.messagebus.service" ;;
        "skills")            _module="yui.skills" ;;
        "audio")             _module="yui.audio" ;;
        "voice")             _module="yui.client.speech" ;;
        "cli")               _module="yui.client.text" ;;
        "audiotest")         _module="yui.util.audio_test" ;;
        "wakewordtest")      _module="test.wake_word" ;;
        "enclosure")         _module="yui.client.enclosure" ;;
...
function launch-background() {
    ...
    if [[ "${1}" == "bus" ]] ; then
        echo "CAUTION: The Yui bus is an open websocket with no built-in security"
    ...
    python3 -m ${_module} $_params >> /var/log/yui/${1}.log 2>&1 &
}

function launch-all() {
    echo "Starting all yui-core services"
    ...
    if [ ! -f .installed ] || ! md5sum -c &> /dev/null < .installed ; then
        ...
        if [ "$auto_update" == "true" ] ; then
            ...
            if command -v notify-send >/dev/null ; then
                ...
                notify-send "Yui Dependencies Outdated" "Run ./dev_setup.sh again"
            fi
            exit 1
        fi
    fi
...
if [[ "${1}" == "restart" ]] || [[ "${_opt}" == "restart" ]] ; then
    ...
    if [[ "${_opt}" == "restart" ]] ; then
        # Support "start-yui.sh restart all" as well as "start-yui.sh all restart"
        _opt=$1
    fi
    shift
fi
...
case ${_opt} in
    ...
    "skillstest")
        ...
        ;;
    "vktest")
        source "$DIR/bin/yui-skill-testrunner" vktest "$@"
        ;;
    "audiotest")
        launch-process ${_opt}
        ;;
